...
server.on("request",(req,res)=>{
    ...
    if(req.url=='/' || req.url=='/index' || req.url=='/index.html'){
        ...
        //请求方式不同,携带参数的方式不同,服务器获取的解析参数的方式也不同
        if(req.method=='GET'){
            //当请求方式为GET时执行代码
            ...
            //检测固定账号和密码,将来此处需要连接数据库,查询表记录以判断是否存在该用户
            ...
            //当请求方式为POST时执行代码
            //从请求体中解析
            //请求对象监听数据传入并接受buffer数据(以字节流的形式传递,需要转换字符传)
            //如果数据量大,回分段接受和传递,接收时也需要拼接最终接收完整
            var str=""
            req.on("data",chunk=>{
                console.log(chunk)
